MODULE TRGTOL_MOD 
  ...
  INTEGER(KIND=JPIM) :: IERROR, irank,ILASTMAX
  LOGICAL :: LLGW
...
!     END IF
   ...
   ZCOMBUFS(-1,INS) = 1
   ZCOMBUFS(0,INS) = IFLD
   !$acc update device (ZCOMBUFS(-1,INS),ZCOMBUFS(0,INS)) 
   ENDDO
  ...
  CALL GSTATS(1603,0)
    !$acc data copyin(JRECV,IRECVTOT,KF_FS,KINDEX,INDOFF,ISEND_FLD_START,ISEND_FLD_END) &
    !$acc&     present(ZCOMBUFR,PGLAT)
  ...
  IF(ISENDTOT(MYPROC) > 0 )THEN
    !!IF( NPROC > 1 )THEN
    !!$ACC if(present(PGPUV)) update host(PGPUV) async(1)
    !!$ACC if(present(PGP))   update host(PGP) async(1)
    !!$ACC if(present(PGP2))  update host(PGP2) async(1)
    !!$ACC if(present(PGP3a)) update host(PGP3a) async(1)
    !!$ACC if(present(PGP3b)) update host(PGP3b) async(1)
    !!ENDIF
    IFLDS = 0
    DO JFLD=1,KF_GP
      IF(KVSET(JFLD) == MYSETV .OR. KVSET(JFLD) == -1) THEN
  ...
  #endif
   !!$ACC data &
   !!$ACC& copyin(IGPTRSEND,IUVLEVS,iuvpars,IGP2PARS,IGP3ALEVS,IGP3BLEVS,IGP3APARS,IGP3BPARS) &
   !!$ACC& copyin(IFLDOFF,INDOFF,IGPTROFF,KINDEX) &
   !!$ACC& copyin(LLUV,LLGP2,LLGP3A,LLGP3B)
  ...
  !!$ACC data &
  !!$ACC present(PGLAT) &
  !!$ACC copyin(ZCOMBUFR(-1:,:),JRECV,IRECVTOT) &
  ...
  DO INR=1,INRECV
      ...
      IRECV_FLD_START = ZCOMBUFR(-1,INR)
      IRECV_FLD_END   = ZCOMBUFR(0,INR)
      !!!!!$ACC parallel loop collapse(2) private(II)
      DO JFLD=IRECV_FLD_START,IRECV_FLD_END
        DO JL=1,ILEN
          II = KINDEX(INDOFF(IRECV)+JL)
          IF (LLGW) THEN
