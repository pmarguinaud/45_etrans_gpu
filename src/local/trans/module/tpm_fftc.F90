...
USE PARKIND1, ONLY: JPIB, JPIM, JPRBT
...
SUBROUTINE CREATE_PLAN_FFT (KPLAN, KTYPE, KN, KLOT, KISTRIDE, KIDIST, KOSTRIDE, KODIST)
...
INTERFACE
  SUBROUTINE CREATE_PLAN_FFTC (KPLAN, KN, KISTRIDE, KIDIST, KOSTRIDE, KODIST, KTYPE, KLOT) &
           & BIND(C,NAME="create_plan_fftc_")
   USE, INTRINSIC :: ISO_C_BINDING
   INTEGER(C_INT) :: KPLAN
...
DO JL=1,TC%N_PLANS(KN)
  IF( KLOT == CURR_FFTC_PLAN%NLOT .AND. KTYPE == CURR_FFTC_PLAN%NTYPE .AND. &
    & IISTRIDE == CURR_FFTC_PLAN%NISTRIDE .AND. IIDIST == CURR_FFTC_PLAN%NIDIST .AND. &
    & IOSTRIDE == CURR_FFTC_PLAN%NOSTRIDE .AND. IODIST == CURR_FFTC_PLAN%NODIST )THEN
    LLFOUND=.TRUE.
...
!     WRITE(*,'("CREATE_PLAN_FFT: END: DESTROYING A PLAN AT THE START OF THE LIST")')
    ENDIF
  ENDIF
  CALL CREATE_PLAN_FFTC(IPLAN, KTYPE=KTYPE, KN=KN, KLOT=KLOT, KISTRIDE=IISTRIDE, &
  ...
  CURR_FFTC_PLAN%NISTRIDE = IISTRIDE
  CURR_FFTC_PLAN%NIDIST   = IIDIST
  CURR_FFTC_PLAN%NOSTRIDE = IOSTRIDE
  CURR_FFTC_PLAN%NODIST   = IODIST
...
!!$OMP CRITICAL
!!$ACC host_data use_device(KPLAN)
...
!   !$ACC host_data use_device(CURR_FFTC_PLAN%NPLAN)
!   CALL DESTROY_PLAN_FFTC(CURR_FFTC_PLAN%NPLAN)
!   !$ACC end host_data
!   CURR_FFTC_PLAN=>CURR_FFTC_PLAN%NEXT_PLAN 
! ENDDO
ENDDO
