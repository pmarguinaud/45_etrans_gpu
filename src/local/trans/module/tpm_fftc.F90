...
SUBROUTINE CREATE_PLAN_FFT(KPLAN,KTYPE,KN,KLOT,KSTRIDE,KDIST)
INTEGER(KIND=JPIM),INTENT(OUT) :: KPLAN
INTEGER(KIND=JPIM),INTENT(IN) :: KTYPE,KN,KLOT,KSTRIDE,KDIST
...
INTERFACE
  SUBROUTINE CREATE_PLAN_FFTC(KPLAN,KTYPE,KN,KLOT,KSTRIDE,KDIST) BIND(C,NAME="create_plan_fftc_")
   USE, INTRINSIC :: ISO_C_BINDING
   INTEGER(C_INT) :: KPLAN
   INTEGER(C_INT) :: KTYPE,KN,KLOT,KSTRIDE,KDIST
...
DO JL=1,TC%N_PLANS(KN)
  IF( KLOT == CURR_FFTC_PLAN%NLOT .AND. KTYPE == CURR_FFTC_PLAN%NTYPE .AND. &
    & KSTRIDE == CURR_FFTC_PLAN%NSTRIDE .AND. KDIST == CURR_FFTC_PLAN%NDIST )THEN
    LLFOUND=.TRUE.
...
!     WRITE(*,'("CREATE_PLAN_FFT: END: DESTROYING A PLAN AT THE START OF THE LIST")')
    ENDIF
  ENDIF
  CALL CREATE_PLAN_FFTC(IPLAN,KTYPE,KN,KLOT,KSTRIDE,KDIST)
  KPLAN=IPLAN
  TC%N_PLANS(KN)=TC%N_PLANS(KN)+1
