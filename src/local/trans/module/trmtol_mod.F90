...
USE MPL_MODULE  ,ONLY : MPL_BARRIER, MPL_ALL_MS_COMM, MPL_WAIT, JP_NON_BLOCKING_STANDARD, MPL_MYRANK
USE MPL_MODULE  ,ONLY : MPL_ALLTOALLV_REAL8
...
INTEGER(KIND=JPIM) :: ITAG, J, ILEN, ISTA, ME
...
IF(NPROC > 1) THEN
  IF (LHOOK) CALL DR_HOOK('TRMTOL_CUDAAWARE_BAR',0,ZHOOK_HANDLE_BAR)
 ! CALL GSTATS_BARRIER(764)
  IF (LHOOK) CALL DR_HOOK('TRMTOL_CUDAAWARE_BAR',1,ZHOOK_HANDLE_BAR)
! IF (LSYNC_TRANS) THEN
...
! ENDIF
  ...
  ME = MPL_MYRANK(MPL_ALL_MS_COMM)
  !$ACC kernels present(PFBUF, PFBUF_IN)
  DO J = IOFFR(ME)+1, IOFFR(ME)+ILENR(ME)+1
    PFBUF(J) = PFBUF_IN(J)
  ENDDO
  !$ACC end kernels
  ...
  ILENS(ME) = 0
  ILENR(ME) = 0
  !TODO figure out why we can't call the interface function
  ...
  CALL GSTATS(807,1)
  IF (LHOOK) CALL DR_HOOK('TRMTOL_CUDAAWARE_BAR2',0,ZHOOK_HANDLE_BAR2)
  !CALL GSTATS_BARRIER2(764)
  IF (LHOOK) CALL DR_HOOK('TRMTOL_CUDAAWARE_BAR2',1,ZHOOK_HANDLE_BAR2)
