...
ALLOCATE (ZGPBUFL (NGPTOT, 3*IFLDGPG, 1))
...
DO ITIME = 1, NTIME
  ...
  IF (LELAM) THEN
    CALL EDIR_TRANS (PSPSCALAR=ZSPBUFL, PGP=ZGPBUFL (:,1:IFLDGPG,1:1), KVSETSC=IVSET)
    CALL EINV_TRANS (PSPSCALAR=ZSPBUFL, PGP=ZGPBUFL, KVSETSC=IVSET, LDSCDERS=.TRUE.)
  ELSE
    CALL DIR_TRANS (PSPSCALAR=ZSPBUFL, PGP=ZGPBUFL (:,1:IFLDGPG,1:1), KVSETSC=IVSET)
    CALL INV_TRANS (PSPSCALAR=ZSPBUFL, PGP=ZGPBUFL, KVSETSC=IVSET, LDSCDERS=.TRUE.)
  ENDIF
...
INTEGER (KIND=JPIM) :: ITO (3*IFLDGPG), JFLD
CHARACTER (LEN=16) :: CLPREF, CLSUFF

ITO = 1

IF (MYPROC == 1) THEN
  ALLOCATE (ZGPBUFG (NGPTOTG, 3*IFLDGPG))

  IF (LELAM) THEN
    CALL EGATH_GRID (PGPG=ZGPBUFG, KFGATHG=3*IFLDGPG, KTO=ITO, PGP=ZGPBUFL (:,:,:))
  ELSE
    CALL GATH_GRID (PGPG=ZGPBUFG, KFGATHG=3*IFLDGPG, KTO=ITO, PGP=ZGPBUFL (:,:,:))
  ENDIF
  ...
  DO JFLD = 1, IFLDGPG
    CLPREF = 'SURF'
    WRITE (CLSUFF, '("FFFX.",I4.4)') JFLD
    CALL WFA (ZGPBUFG (:, 2*IFLDGPG+JFLD), CDFILE, CLPREF, CLSUFF)
  ENDDO

  DO JFLD = 1, IFLDGPG
    CLPREF = 'SURF'
    WRITE (CLSUFF, '("FFFY.",I4.4)') JFLD
    CALL WFA (ZGPBUFG (:, 1*IFLDGPG+JFLD), CDFILE, CLPREF, CLSUFF)
  ENDDO

  DEALLOCATE (ZGPBUFG)
ELSE
  IF (LELAM) THEN
    CALL EGATH_GRID (KFGATHG=3*IFLDGPG, KTO=ITO, PGP=ZGPBUFL (:,:,:))
  ELSE
    CALL GATH_GRID (KFGATHG=3*IFLDGPG, KTO=ITO, PGP=ZGPBUFL (:,:,:))
  ENDIF
ENDIF
...
IF (MYPROC == 1) THEN
  ...
  ELSE
  BLOCK
    REAL (KIND=JPRB) :: ZLON, ZLAT, ZVAL, ZX, ZY, ZVALDX, ZVALDY
...
!         ZVALDY = + 6. * RPI * COS (4. * RPI * ZX) * COS (6. * RPI * ZY)
        ...
        ENDIF
        ZGPBUFG (JGLO, 1) = ZVAL
        ZGPBUFG (JGLO, 2) = ZVALDY
        ZGPBUFG (JGLO, 3) = ZVALDX
        JGLO = JGLO + 1
      ENDDO
    ENDDO
  ENDBLOCK
  ...
  IF (LELAM) THEN
    CALL EDIST_GRID (PGPG=ZGPBUFG, KFDISTG=3_JPIM, KFROM=[1_JPIM,1_JPIM,1_JPIM], PGP=ZGPBUFL (:,1:3,:))
  ELSE
    CALL DIST_GRID (PGPG=ZGPBUFG, KFDISTG=3_JPIM, KFROM=[1_JPIM,1_JPIM,1_JPIM], PGP=ZGPBUFL (:,1:3,:))
  ENDIF
...
ELSE
  IF (LELAM) THEN
    CALL EDIST_GRID (KFDISTG=3_JPIM, KFROM=[1_JPIM,1_JPIM,1_JPIM], PGP=ZGPBUFL (:,1:3,:))
  ELSE
    CALL DIST_GRID (KFDISTG=3_JPIM, KFROM=[1_JPIM,1_JPIM,1_JPIM], PGP=ZGPBUFL (:,1:3,:))
  ENDIF
ENDIF

ZGPBUFL (:, 1*IFLDGPG+1, :) = ZGPBUFL (:, 2, :)
ZGPBUFL (:, 2*IFLDGPG+1, :) = ZGPBUFL (:, 3, :)

DO IFLD = 2, IFLDGPG
  ZGPBUFL (:, IFLD, 1) = ZGPBUFL (:, 1, 1) * REAL (IFLD, JPRB)
  ZGPBUFL (:, 1*IFLDGPG+IFLD, 1) = ZGPBUFL (:, 1*IFLDGPG+1, 1)
  ZGPBUFL (:, 2*IFLDGPG+IFLD, 1) = ZGPBUFL (:, 2*IFLDGPG+1, 1)
ENDDO
