...
!        A.Bogatchev  16-09-2010 Intruducing of LGPNORM
...
IF(IUBOUND(1) < NPROMA) THEN
  WRITE(NOUT,*)'EGPNORM_TRANS:FIRST DIM. OF PGP TOO SMALL ',IUBOUND(1),NPROMA
  CALL ABORT_TRANS('EGPNORM_TRANS:FIRST DIMENSION OF PGP TOO SMALL ')
ENDIF
IF(IUBOUND(2) < KFIELDS) THEN
  WRITE(NOUT,*)'EGPNORM_TRANS:SEC. DIM. OF PGP TOO SMALL ',IUBOUND(2),KFIELDS
  CALL ABORT_TRANS('EGPNORM_TRANS:SECOND DIMENSION OF PGP TOO SMALL ')
ENDIF
IF(IUBOUND(3) < NGPBLKS) THEN
  WRITE(NOUT,*)'EGPNORM_TRANS:THIRD DIM. OF PGP TOO SMALL ',IUBOUND(3),NGPBLKS
  CALL ABORT_TRANS('EGPNORM_TRANS:THIRD DIMENSION OF PGP TOO SMALL ')
ENDIF
...
!$OMP END PARALLEL DO
  ...
  DO JGL=IBEG,IEND
    IGL = D%NPTRLS(MYSETW) + JGL - 1
    DO JF=1,IF_FS
      ZAVE(JF,JGL)=ZAVE(JF,JGL)/(G%NLOEN(IGL)*G%NLOEN(IGL))
    ENDDO
  ENDDO
  ...
  DO JSETV=2,NPRTRV
    ...
    IF(ILEN > 0)THEN
      ...
      CALL MPL_RECV(ZRCV(:),KSOURCE=NPRCIDS(IPROC),KTAG=ITAG,&
        &KMP_TYPE=JP_BLOCKING_STANDARD,KOUNT=ILENR,CDSTRING='EGPNORM_TRANS:V')
      IF(ILENR /= ILEN)THEN
        CALL ABOR1('EGPNORM_TRANS:ILENR /= ILEN')
      ENDIF
      IND=0
...
ELSE
  ...
  IF(ILEN > 0)THEN
    ...
    CALL MPL_SEND(ZSND(:),KDEST=NPRCIDS(IPROC),KTAG=ITAG,&
      &KMP_TYPE=JP_BLOCKING_STANDARD,CDSTRING='EGPNORM_TRANS:V')
    DEALLOCATE(ZSND)
  ENDIF
  ...
  IF( MYSETW == 1 )THEN
    ...
    DO JSETW=2,NPRTRW
      ...
      IF(ILEN > 0 )THEN
        ...
        CALL MPL_RECV(ZRCV(:),KSOURCE=NPRCIDS(IPROC),KTAG=ITAG,&
          &KMP_TYPE=JP_BLOCKING_STANDARD,KOUNT=ILENR,CDSTRING='EGPNORM_TRANS:W')
        IF(ILENR /= ILEN)THEN
          CALL ABOR1('EGPNORM_TRANS:ILENR /= ILEN')
        ENDIF
        IND=0
