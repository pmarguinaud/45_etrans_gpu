...
DO KMLOC = 1, D%NUMP
  ...
  IF(PRESENT(KFLDPTR)) THEN
    DO JFLD=1,KFIELDS
      ...
      DO J=1,ILCM,2
        INM = IOFF+(J-1)*2
        PFFT(J  ,IR,KMLOC) = PSPEC(IFLD,INM  )
        PFFT(J+1,IR,KMLOC) = PSPEC(IFLD,INM+1)
        PFFT(J  ,II,KMLOC) = PSPEC(IFLD,INM+2)
        PFFT(J+1,II,KMLOC) = PSPEC(IFLD,INM+3)
      ENDDO
    ENDDO
  ...
  ELSE
    DO J=1,ILCM,2
      ...
      DO JFLD=1,KFIELDS
        IR = 2*(JFLD-1)+1
        II = IR+1
        PFFT(J  ,IR,KMLOC) = PSPEC(JFLD,INM  )
        PFFT(J+1,IR,KMLOC) = PSPEC(JFLD,INM+1)
        PFFT(J  ,II,KMLOC) = PSPEC(JFLD,INM+2)
        PFFT(J+1,II,KMLOC) = PSPEC(JFLD,INM+3)
      ENDDO
    ENDDO
