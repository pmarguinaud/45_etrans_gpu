...
REAL(KIND=JPRB),TARGET  :: ZDUM(D%NLENGTF,1)
...
REAL(KIND=JPRB),TARGET  :: ZGTF(D%NLENGTF,KF_FS)
...
IF(KF_UV > 0 .OR. KF_SCDERS > 0) THEN
  CALL ABOR1 ('EFSC BROKEN')
! CALL EFSC(KF_UV,KF_SCALARS,KF_SCDERS,IST_UV,IST_SC,IST_NSDERS,IST_EWDERS,IST_UVDERS)
ENDIF
