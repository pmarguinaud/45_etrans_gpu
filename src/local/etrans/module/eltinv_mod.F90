...
REAL(KIND=JPRB) :: ZFFT(RALD%NDGLSUR+R%NNOEXTZG,KLEI2,D%NUMP)
...
IF (KF_UV > 0) THEN
  ...
  IVL   = 6*KF_UV+1
  IVU   = 8*KF_UV
! CALL EPRFI1B(KM,ZFFT(:,IVORL:IVORU),PSPVOR,KF_UV,KFLDPTRUV)
! CALL EPRFI1B(KM,ZFFT(:,IDIVL:IDIVU),PSPDIV,KF_UV,KFLDPTRUV)
! ILAST = ILAST+4*KF_UV
! CALL EVDTUV(KM,KF_UV,KFLDPTRUV,ZFFT(:,IVORL:IVORU),ZFFT(:,IDIVL:IDIVU),&
