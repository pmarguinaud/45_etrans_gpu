...
USE ELTDATA_MOD      ,ONLY : ZFFT_PERM
...
!REAL(KIND=JPRB) :: ZFFT(RALD%NDGLSUR+R%NNOEXTZG,D%NUMP,KLEI2)
...
& .OR. (UBOUND (ZFFT_PERM, 3) < KLEI2)) THEN
...
IF (.NOT. ALLOCATED (ZFFT_PERM)) THEN
  ALLOCATE (ZFFT_PERM (RALD%NDGLSUR+R%NNOEXTZG,D%NUMP,KLEI2))
