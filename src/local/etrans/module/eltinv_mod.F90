MODULE ELTINV_MOD
CONTAINS
SUBROUTINE ELTINV(KF_OUT_LT,KF_UV,KF_SCALARS,KF_SCDERS,KLEI2,KDIM1,&
...
IF(KF_SCALARS > 0)THEN
  IF(PRESENT(PSPSCALAR)) THEN
    ...
    CALL EPRFI1B(ZFFA(:,IFIRST:ILAST,:),PSPSCALAR(:,:),KF_SCALARS,KFLDPTRSC)
    !$acc update device (ZFFA(:,IFIRST:ILAST,:))
  ELSE
    CALL ABOR1 ('')
    IF(PRESENT(PSPSC2) .AND. NF_SC2 > 0) THEN
      IFIRST = ILAST+1
      ILAST  = IFIRST-1+2*NF_SC2
!     CALL EPRFI1B(ZIA(:,IFIRST:ILAST),PSPSC2(:,:),NF_SC2)
    ENDIF
...
!       CALL EPRFI1B(ZIA(:,IFIRST:ILAST),PSPSC3A(:,:,J3),IDIM1)
      ENDDO
    ENDIF
    IF(PRESENT(PSPSC3B) .AND. NF_SC3B > 0) THEN
      ...
      DO J3=1,IDIM3
        IFIRST = ILAST+1
        ILAST  = IFIRST-1+2*IDIM1
!       CALL EPRFI1B(ZIA(:,IFIRST:ILAST),PSPSC3B(:,:,J3),IDIM1)
      ENDDO
    ENDIF
  ENDIF
...
CALL ELEINV(IFC,KF_OUT_LT,ZFFA) 
