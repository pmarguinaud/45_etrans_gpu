...
USE TPMALD_DISTR    ,ONLY : DALD, DALD_NCPL2M, DALD_NPME, DALD_NESM0
...
IF(.NOT.D%LGRIDONLY) THEN
  ...
  ENDDO
  ALLOCATE (DALD_NPME (0:RALD%NMSMAX))
  DALD_NPME = DALD%NPME
  !$acc enter data create (DALD_NPME)
  !$acc update device (DALD_NPME)
  DO JM=0,RALD%NMSMAX
    DALD%NCPL2M(JM) = 2*(IKNTMP(JM)+1)
  ENDDO
  ALLOCATE (DALD_NCPL2M (0:RALD%NMSMAX))
  DALD_NCPL2M = DALD%NCPL2M
  !$acc enter data create (DALD_NCPL2M)
  !$acc update device (DALD_NCPL2M)
  ...
  ALLOCATE (FALD_RLEPINM (R%NSPEC_G/2))
  FALD_RLEPINM = FALD%RLEPINM
  !$acc enter data create (FALD_RLEPINM)
  !$acc update device (FALD_RLEPINM)
  D%NUMPP(:) = 0
  ISPEC(:) = 0
  DALD%NESM0(:)=-99
  ...
  DO JM=0,RALD%NMSMAX
    ...
    ENDIF
  ENDDO
  ALLOCATE (DALD_NESM0(0:RALD%NMSMAX))
  DALD_NESM0 = DALD%NESM0
  !$acc enter data copyin (DALD_NESM0)
  D%NPOSSP(1) = 1
  ISPEC2P = 4*ISPEC(1)
  D%NSPEC2MX = ISPEC2P
  DO JA=2,NPRTRW
