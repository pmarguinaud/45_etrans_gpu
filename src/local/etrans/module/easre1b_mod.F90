...
!$acc parallel loop collapse (3) private (JMLOC, JGL, JFLD, IPROC, IISTAN) &
!$acc& present (FOUBUF_IN, PFFT, D_NSTAGT0B, D_NPNTGTB1, D_NPROCL, D_NUMP, R_NDGL)
DO JMLOC = 1, D_NUMP
  DO JGL=1,R_NDGL
    DO JFLD  =1,2*KFIELD
      IPROC=D_NPROCL(JGL)
      IISTAN=(D_NSTAGT0B(IPROC) + D_NPNTGTB1(JMLOC,JGL))*2*KFIELD
      FOUBUF_IN(IISTAN+JFLD)=PFFT(JGL,JFLD,JMLOC)
    ENDDO
  ENDDO
ENDDO
